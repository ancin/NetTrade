var Global = function() {
	...
	return {
		init : function() {
			...
			var b = $('.page-sidebar .menu-root a[data-code="MMOBILE"]')
			...
			$.address
					.change(function(g) {
						App.scrollTop();
						var h = $('.page-sidebar li > a.ajaxify[rel="address:'
								+ g.value + '"]');
						if (h.size() > 0) {
							...
							if (g.value == "/" || g.value == "/dashboard") {
								...
								if (g.value == "/lock") {
									...
									Global.addOrActivePanel(h, WEB_ROOT
											+ g.value)
								}
							}
						}
			...
			jQuery("body").on(
					...
					function(j) {
						...
						if (h.length > 0) {
							h.parent(".tab-content").parent().find(
					...
					.on(
							...
							function(k) {
								...
								if (j.attr("data-url")) {
									...
									if (jQuery().jqGrid) {
										i.find("table.ui-jqgrid-btable").each(
												function() {
													var l = $(this);
					...
					.on(
							...
							function(l) {
								...
								if (k.hasClass("disabled")
										|| k.attr("data-tab-disabled") == "true") {
									return false
								}
								var g = k.attr("href");
								var j = new RegExp("^#");
								if (k.attr("data-url") == undefined
										&& !j.test(g)) {
									k.attr("data-url", g);
									var m = "tab_content_" + Util.hashCode(g);
									k.attr("href", "#" + m);
									var i = k.closest(
											"div.tabbable,div.portlet-tabs")
											.find(" > div.tab-content");
									if (i.length == 0) {
										i = $('<div class="tab-content">')
												.appendTo(
														k
																.closest("div.tabbable"))
									}
									var h = i.find("div#" + m);
									if (h.length == 0) {
										h = $(
			...
			jQuery("body").on(
					...
					function(h) {
						...
						if (g) {
							f.find("input[name='attachmentName']").val(
									"_attachment_" + g)
						}
			...
			jQuery("body").on("click", ".btn-post-url", function(i) {
				...
				h.ajaxPostURL({
					url : g,
					success : j,
					confirmMsg : h.attr("data-confirm")
				})
			});
		...
		findUserProfileParams : function(b) {
			if (a == undefined || a == null) {
				a = $("body").cacheData(
						WEB_ROOT + "/profile/simple-param-val!params.json")
			}
			return a[b]
		},
		...
		autoCloseContainer : function(i, g) {
			var d = $(i);
			if (d.attr("data-prevent-close") == undefined
					|| d.attr("data-prevent-close") == "false") {
				...
				if (e.length == 0) {
					...
					if (j.size() > 0) {
						...
						if (l.length > 0) {
							l.parent(".tab-content").parent().find(
									" > .nav li.active .close").click()
						} else {
					...
					if (e.find(" > ul.nav > li").not(".tools").size() < 2) {
						...
						var c = d.closest("form").find("input[name='id']")
								.val();
						if (c && c != "") {
							...
							var b = Util.AddOrReplaceUrlParameter(f
									.attr("data-url"), "id", g.userdata.id);
							f.attr("data-url", b);
		...
		addOrActivePanel : function(e, c) {
			...
			if (i.length == 0) {
				...
				var m = '<li><a href="' + c + '" title="刷新当前页面">' + f
						+ "</a></li>";
				if (b.length > 0) {
					var k = b.parent("li").parent(".sub-menu");
					while (k.length > 0) {
						var f = k.prev("a").children("span.title").text();
						m = '<li class="hidden-inline-xs"><a href="#" title="TODO">'
								+ f
				...
				m = '<li><a href="#dashboard" class="btn-dashboard"><i class="fa fa-home"></i></a></li> '
						+ m;
				i.data("path", m)
			}
			var j = 1;
			...
			h.find("> li:not(.btn-group) > a[href='" + c + "']").click(
					function(n) {
						n.preventDefault();
						l.ajaxGetUrl(c)
					})
