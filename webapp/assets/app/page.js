var Page = function() {
				...
				.each(
						function() {
							...
							if (pkValue) {
								...
								$op.val("update");
								thumbnail.append($op);
								var $idx = $('<input type="hidden" name="'
										+ name.substring(0, name.indexOf("]"))
										+ "]." + indexProp + '" />');
																							...
																							.each(
																									...
																									function(
																										...
																										var $new = $('<input type="hidden" name="'
																												+ name
																												+ '" data-multiimage="edit"  data-index-prop="'
																												+ indexProp
																												+ '" />');
																										$new
																												.val(data.md5);
	...
	return {
		initAjaxBeforeShow : function($container) {
			...
			$("[data-profile-param]:not([data-profile-param-done])", $container)
					.each(
							function() {
								var $el = $(this);
								...
								var controlDoneFlag = "data-profile-param-done"
										+ code;
								if ($controls.attr(controlDoneFlag)) {
									return
								}
								$controls.attr(controlDoneFlag, true);
								var id = $el.closest("form").find(
										"input[name='id']").val();
								var orgVal = $el.val();
								if (id.length == 0
										&& (orgVal == undefined || orgVal == "")) {
									var initVal = Global
											.findUserProfileParams(code);
									if (initVal) {
										if ($el.is('[type="radio"]')) {
											$controls.find(
													'[type="radio"][value="'
													...
													$btn
															.click(function() {
																...
																$btn
																		.ajaxPostURL({
																			url : WEB_ROOT
																					+ "/profile/simple-param-val!doSave",
																			success : function() {
																				$
																						.each(
																								codes,
			...
			$("input.input-daterangepicker", $container).each(
					function() {
						var $daterangepicker = $(this);
						var options = $.extend(true,
								$.fn.daterangepicker.defaults, $daterangepicker
										.attr("data-daterangepicker"));
						$daterangepicker.daterangepicker(options, function(
								start, end) {
			...
			$('[data-toggle="dropdownselect"]', $container)
					.each(
							function() {
								...
								$elems
										.wrapAll('<div class="input-icon right"></div>');
								...
								if ($dropdown.attr("data-minWidth")) {
									$container.css("min-width", $dropdown
											.attr("data-minWidth"))
								}
								$dropdown
										.add(
			...
			$(".tabbable > .nav[data-active-index]").each(
					function() {
						var $nav = $(this);
						...
						var curActiveIdx = $nav.find("li:not(.tools)").index(
								$nav.find("li.active"));
						if (idx != curActiveIdx) {
							$nav.find("li:not(.tools):eq(" + idx + ") > a")
									.click()
						}
					});
			$("a.x-editable")
							...
							function() {
								...
								$edit
										.editable({
											...
											success : function() {
												if ($edit
														.hasClass("editable-bpm-task-transfer")) {
													$(
															"#layout-nav .btn-close-active")
															.click();
													$(".ajaxify-tasks")
															.ajaxGetUrl(
																	$(
					...
					.each(
							function() {
								...
								if ($el.attr("data-singleimage-width")) {
									options.width = $el
											.attr("data-singleimage-width")
								}
								if ($el.attr("data-singleimage-height")) {
									options.height = $el
											.attr("data-singleimage-height")
								}
								...
								thumbnail.delegate("a.btn-view", "click",
										function() {
											window.open(thumbnail.find("img")
													.attr("src"), "_blank")
										});
								$el.attr("singleimage-done", true)
							});
			$("a.btn-fileinput-trigger", $container)
					.each(
							function() {
								var $btn = $(this);
								...
								if (!BooleanUtil.toBoolean(readonly)) {
									var $hidden = $(
											'<input type="hidden" name="_attachment_'
													+ category
													+ '"  value="" />')
											.insertBefore($btn)
								}
						...
						map.addEventListener("click", function(e) {
							...
							myGeo.getLocation(point,
									function(rs) {
										...
										rs.fullAddress = addComp.province
												+ addComp.city
												+ addComp.district
												+ addComp.street
