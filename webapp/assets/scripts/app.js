var App = function() {
	...
	var d = function() {
		...
		var M = $("body");
		var L;
		if (M.hasClass("page-footer-fixed") === true
				&& M.hasClass("page-sidebar-fixed") === false) {
	...
	var D = function() {
		...
		$(".page-sidebar").off("mouseenter").on(
				...
				function() {
					var L = $("body");
					if ((L.hasClass("page-sidebar-closed") === false || L
							.hasClass("page-sidebar-fixed") === false)
							|| $(this).hasClass("page-sidebar-hovering")) {
						return
					}
					L.removeClass("page-sidebar-closed").addClass(
		...
		$(".page-sidebar").off("mouseleave").on(
				...
				function() {
					var L = $("body");
					if ((L.hasClass("page-sidebar-hover-on") === false || L
							.hasClass("page-sidebar-fixed") === false)
							|| $(this).hasClass("page-sidebar-hovering")) {
						return
					}
	...
	var v = function() {
		...
		$(".page-sidebar, .header").on(
				...
				function(O) {
					var M = $("body");
					var N = $(".page-sidebar");
					if ((M.hasClass("page-sidebar-hover-on") && M
							.hasClass("page-sidebar-fixed"))
							|| N.hasClass("page-sidebar-hovering")) {
	...
	var J = function() {
		jQuery("body").on("click",
				".portlet > .portlet-title > .tools > a.remove", function(L) {
					L.preventDefault();
		...
		jQuery("body")
				.on(
						...
						function(M) {
							M.preventDefault();
							var L = jQuery(this).closest(".portlet").children(
									".portlet-body");
	...
	var i = function() {
		...
		jQuery("body").on("show.bs.collapse", ".accordion.scrollable",
				function() {
	...
	var I = function() {
		...
		if (location.hash) {
			...
			$('a[href="#' + L + '"]').parents(".tab-pane:hidden").each(
					function() {
						var M = $(this).attr("id");
						$('a[href="#' + M + '"]').click()
					});
	...
	var a = function() {
		...
		var M = function(Q) {
			$("#style_color").attr("href", "assets/css/themes/" + Q + ".css");
		...
		$(".layout-option, .header-option, .sidebar-option, .footer-option", L)
				.change(O);
	...
	return {
		...
		getURLParameter : function(N) {
			var L = window.location.search.substring(1), M, P, O = L.split("&");
