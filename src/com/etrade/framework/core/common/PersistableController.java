...
public abstract class PersistableController<T extends PersistableEntity<ID>, ID extends Serializable>
    ...
    protected HttpHeaders doDelete() {
        ...
        if (rejectSize == 0) {
            ...
            if (rejectSize == entities.size()) {
                ...
                setModel(OperationResult.buildWarningResult("删除操作已处理. 成功:"
                                                            + (entities.size() - rejectSize) + "条"
                                                            + ",失败:" + rejectSize + "条",
                    errorMessageMap));
            }
        }
    ...
    protected HttpHeaders processBatchEntities(String op,
        ...
        int rejectSize = errorMessageMap.size();
        if (rejectSize == 0) {
            setModel(OperationResult.buildSuccessResult("成功" + op + "所选选取记录:" + entities.size()
                                                        + "条"));
        } else {
            if (rejectSize == entities.size()) {
                setModel(OperationResult
                    .buildFailureResult("所有选取记录" + op + "操作失败", errorMessageMap));
            } else {
                setModel(OperationResult.buildWarningResult(op + "操作已处理. 成功:"
                                                            + (entities.size() - rejectSize) + "条"
                                                            + ",失败:" + rejectSize + "条",
                    errorMessageMap));
            }
